...
class PagesController extends \BaseController
{
    ...
    public function getIndex()
    {
    	...
    	$this->layout->content = View::make('pages.index')->with('pages', $pages)->with('companies', $companies)->with('user_role', $user_role)->with('count', $count)->with('company_id', $companies->id);
    ...
    public function postAdd()
    {
    	...
    	if ($validator->passes()) {
    		...
    		$this->layout->content = View::make('pages.preview')->with('content', $content);
    	} else {
    ...
    public function postEdit()
    {
    	...
    	if ($validator->passes()) {
            ...
            if ($page_id == 1) { //HOME EDIT
            	...
            	$this->layout->content = View::make('pages.preview_edit_home')->with('content', $content);
            } else {
            	$this->layout          = View::make('layouts.pages');
            	$this->layout->content = View::make('pages.preview_edit')->with('content', $content);
            }
    ...
    public function getPreviewEdit()
    {
    	...
    	if (Session::get('data_session')) {
            ...
            if ($page->save()) { // Save the user and redirect to freelancers home
            	$temp_folder = glob('img/tmp/*'); // get all file names
